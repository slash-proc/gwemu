...
static QEMUSnapshotInfo *gwemu_snapshots_metadata = NULL;
static GwemuSnapshotData *gwemu_snapshots_extra_data = NULL;
static int gwemu_snapshots_len = 0;
static bool gwemu_snapshots_dirty = true;
...
static void gwemu_snapshots_load_data(BlockDriverState *bs_ro,
                                     QEMUSnapshotInfo *info,
                                     GwemuSnapshotData *data, Error **err)
{
    data->disc_path = NULL;
    ...
    if (be32_to_cpu(header[0]) != GWEMU_SNAPSHOT_DATA_MAGIC ||
        be32_to_cpu(header[1]) != GWEMU_SNAPSHOT_DATA_VERSION) {
        return;
    }
    ...
    if (thumbnail_size) {
        ...
        if (gwemu_snapshots_load_png_to_texture(thumbnail, &buf[offset],
                                               thumbnail_size)) {
...
static void gwemu_snapshots_all_load_data(QEMUSnapshotInfo **info,
                                         GwemuSnapshotData **data,
                                         int snapshots_len, Error **err)
{
    ...
    if (*data) {
        for (int i = 0; i < gwemu_snapshots_len; ++i) {
    ...
    *data =
        (GwemuSnapshotData *)g_malloc(sizeof(GwemuSnapshotData) * snapshots_len);
    memset(*data, 0, sizeof(GwemuSnapshotData) * snapshots_len);
    ...
    for (int i = 0; i < snapshots_len; ++i) {
        gwemu_snapshots_load_data(bs_ro, (*info) + i, (*data) + i, err);
        if (*err) {
            break;
        }
    }
                        ...
                        Error **err)
{
    ...
    if (!gwemu_snapshots_dirty && gwemu_snapshots_extra_data &&
        gwemu_snapshots_metadata) {
        goto done;
    }

    if (gwemu_snapshots_metadata)
        g_free(gwemu_snapshots_metadata);
    ...
    snapshots_len = bdrv_snapshot_list(bs, &gwemu_snapshots_metadata);
    gwemu_snapshots_all_load_data(&gwemu_snapshots_metadata,
                                 &gwemu_snapshots_extra_data, snapshots_len,
                                 err);
    if (*err) {
        return -1;
    }

    gwemu_snapshots_len = snapshots_len;

done:
    if (info) {
        *info = gwemu_snapshots_metadata;
    }

    if (extra_data) {
        *extra_data = gwemu_snapshots_extra_data;
    }

    return gwemu_snapshots_len;
}
...
void gwemu_snapshots_save_extra_data(QEMUFile *f)
{
    ...
    void *thumbnail_buf = gwemu_snapshots_create_framebuffer_thumbnail_png(&thumbnail_size);

    qemu_put_be32(f, GWEMU_SNAPSHOT_DATA_MAGIC);
    qemu_put_be32(f, GWEMU_SNAPSHOT_DATA_VERSION);
...
bool gwemu_snapshots_offset_extra_data(QEMUFile *f)
{
    ...
    v = qemu_get_be32(f);
    if (v != GWEMU_SNAPSHOT_DATA_MAGIC) {
        qemu_file_skip(f, -4);
        return true;
    }
